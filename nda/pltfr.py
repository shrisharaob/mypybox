...
def plotfr(filename, axE, axI, filetag, filetype = 'tuning', NE = 20000, NI = 20000, nBins = 100):
    if filetype == 'tuning':
        print '@tuning!'
...
#    print filename, filetag, filetype, NE, NI, nBins
#    plotfr(filename, filetag, filetype, int(NE), int(NI), int(nBins))
    figE, axE = plt.subplots()
    figI, axI = plt.subplots()
    plotfr(filename, axE, axI, filetag, filetype, int(NE), int(NI), int(nBins))
