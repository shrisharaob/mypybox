...
[foldername, NE, NI, xi, nTheta, contrast, alpha, tau_syn, simDuration, nTrials] = DefaultArgs(sys.argv[1:], ['', 20000, 20000, '0.8', 8, 100.0, 0, 3.0, 100000, 1])
fldr = foldername
...
for kk, kTheta in enumerate(theta):
    fn = 'firingrates_xi%s_theta%s_0.%s0_3.0_cntrst100.0_%s_tr0.csv'%(xi, int(kTheta), int(alpha), int(simDuration))
    print foldername + fn
...
np.save(basefolder + '/db/data/tuningCurves_' + fldr + 'rho' + rho + '_xi'+xi, tuningCurves)
