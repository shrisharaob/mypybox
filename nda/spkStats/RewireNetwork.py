...
if __name__ == '__main__':
    ...
    [dbName, NetworkType, K, NE, NI, thetaSig] = DefaultArgs(sys.argv[1:], ['', 'ori', 1000, 10000, 10000, 0.5])
    NE = int(NE)
    NI = int(NI)
    cprob = np.zeros((NE + NI, NE + NI))
    print 'Network type: ', NetworkType
    print 'generating connection probabilities'
    if NetworkType == 'ori':
        ...
        cprob[:NE, :NE] = float(K) / NE
    cprob[:NE, NE:] = float(K) / NE
    cprob[NE:, :NE] = float(K) / NI
    cprob[NE:, NE:] = float(K) / NI
...
#    convec[:NE, :NE] = cprob[:NE, :NE] > np.random.uniform(size = (cprob[:NE, :NE].shape))
    ...
    #     plt.hist(poe[cprob[idx, :]])
    #     plt.title("PO =  %s"%(poe[idx]))
