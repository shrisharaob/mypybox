...
fr = np.loadtxt(bf + 'bidir/i2i/p8/firingrates_xi0.8_theta0_0.80_6.0_cntrst100.0_6000_tr_vm.csv')
fr1 = np.loadtxt(bf + 'bidir/i2i/p8/firingrates_xi0.8_theta0_0.80_12.0_cntrst100.0_6000_tr_vm.csv')
vm = np.loadtxt(bf + 'bidir/i2i/p8/vm_xi0.8_theta0_0.80_6.0_6000_tr_vm.csv')
vm1 = np.loadtxt(bf + 'bidir/i2i/p8/vm_xi0.8_theta0_0.80_12.0_6000_tr_vm.csv')
fg, ax = plt.subplots()
...
figFormat = 'eps'
for i in range(20):
    ax.plot(vm[:, 0], vm[:, idx[i] + 1], 'k', linewidth = 0.5, label = r'$\tau_{s} = 6ms(%.4sHz)$'%(fr[idx[i]]))
    ax1.plot(vm1[:, 0], vm1[:, idx1[i] + 1], 'k',linewidth = 0.5, label = r'$\tau_{s} = 12ms(%.4sHz)$'%(fr1[idx1[i]]))
