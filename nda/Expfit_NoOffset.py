...
p = [0, 1, 2, 3, 4, 5, 6, 7, 75, 8, 82, 84, 85, 86, 88, 9, 95, 97]
realp = [0, .1, .2, .3, .4, .5, .6, .7, .75, .8, .82, .84, .85, .86, .88, .9, .95, .97]
...
#        filenames = ['long_tau_vs_ac_mat_tr10_bidir_%s_bidirI2I_tau%s_p%s.npy'%(taudecayFiletag, kTau, ip) for ip in p]
        ...
        p = RemoveIdicesFromList(p, indices2Remove)
        realp = RemoveIdicesFromList(realp, indices2Remove)
        p_list.append(realp)
        estTau_dec = np.zeros((len(p), ))
        estTau_error = np.zeros((len(p), ))
        for idxp, bidirp in enumerate(p):
            print 'loading file: ', filenames[idxp]
            ac = np.load(dataFolder + filenames[idxp])
            tmpMaxAC = np.argmax(ac[:, neuronType])
            print ac.shape
            if kTau == 3:
                validStartIdx = min([validStartIdxCnst, tmpMaxAC])
            else:
                validStartIdx = 15 #min([validStartIdxCnst, tmpMaxAC]) 
            if ac.ndim == 2:
...
#        plt.plot(realp, estTau_dec[kk, :], 'r.-', label = r'$\tau_s = %sms$'%(kTau), linewidth = 0.2)
    ...
    plt.xlabel('p')
    pdf.Print2Pdf(plt.gcf(), figFolder + 'summary_0_%s'%(neuronTag), paperSize, figFormat = 'eps', IF_ADJUST_POSITION = True,  axPosition = axPos, labelFontsize = 20, tickFontsize = 12)
...
#        plt.plot(realp, estTau_dec[kk, :] / float(kTau), 'k.-', label = r'$\tau_s = %sms$'%(kTau), linewidth = 0.2)
 ...
 #   plt.legend(loc = 0)    
    ...
    plt.xticks([0, 0.5, 1.0])
    plt.xlabel('p')    
    pdf.Print2Pdf(plt.gcf(), figFolder + 'summary_1_%s'%(neuronTag), paperSize, figFormat = 'eps', IF_ADJUST_POSITION = True,  axPosition = axPos, labelFontsize = 12, tickFontsize = 10)
    plt.close('all')
    plt.figure()
    axPos = [0.28, 0.28, .65, 0.65] #[0.25, 0.3, .65, .65]    
    for kk, kTau in enumerate(tau_s):        
#        plt.loglog(1.0 - np.array(realp), estTau_dec[kk, :] / float(kTau),'k.-', linewidth = 0.2)
    ...
    plt.xlabel('1-p')
    plt.ylabel(r'$\tau_{dec} / \tau_s$')
    pdf.Print2Pdf(plt.gcf(), figFolder + 'summary_loglog_normalized_by_tau_s_%s'%(neuronTag), paperSize, figFormat = 'eps', IF_ADJUST_POSITION = True,  axPosition = axPos, labelFontsize = 10, tickFontsize = 10)
    ...
    plt.xlabel('1-p')
    plt.ylabel(r'$\tau_{dec}(ms)$')    
    pdf.Print2Pdf(plt.gcf(), figFolder + 'summary_loglog_%s'%(neuronTag), paperSize, figFormat = 'eps', IF_ADJUST_POSITION = True,  axPosition = axPos, labelFontsize = 20, tickFontsize = 12)        
...
plt.errorbar(td[0][0][:-1], td[1][0][:-1]/3.0, yerr = td[2][0][:-1], fmt = '.k-', ecolor = 'k', elinewidth = 0.2, linewidth = 0.2, markersize = 2.0, markeredgewidth = 0.1)
td = np.load('est_tau_dec_I.npy')
plt.errorbar(td[0][0][:-1], td[1][0][:-1]/3.0, yerr = td[2][0][:-1], fmt = '.r-', ecolor = 'r', elinewidth = 0.2, linewidth = 0.2, markersize = 2.0 , markeredgewidth = 0.1)
plt.xlabel('p')
plt.ylabel(r'$\tau_{dec} / \tau_s$')
plt.yticks([0, 35, 70])
plt.xticks([0, 0.5, 1.0])
